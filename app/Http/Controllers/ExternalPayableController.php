...
class ExternalPayableController extends Controller
{
    ...
    {
        $filters = $request->all() ?: Session::get('external_payables.index_filters', []);
        Session::put('external_payables.index_filters', $filters);
        ...
        return Inertia::render('Debts/ExternalPayables/Index', [
            'items' => $items,
            'companies' => $companies,
            'branches' => $branches,
            'partners' => $partners,
    ...
    {
        $filters = Session::get('external_payables.index_filters', []);
        ...
        if ($request->company_id) {
            ...
            $defaultDebtAccountId = $company?->default_payable_account_id;
        }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->input('create_another', false)) {
            return redirect()->route('external-payables.create')
                ->with('success', 'Hutang eksternal berhasil dibuat.');
        }

        return redirect()->route('external-payables.show', $debt->id)
            ->with('success', 'Hutang eksternal berhasil dibuat.');
    }
    ...
    public function show(Request $request, ExternalDebt $debt)
    {
        ...
        return Inertia::render('Debts/ExternalPayables/Show', [
            'item' => $debt,
            'filters' => $filters,
            'statusStyles' => ExternalDebt::statusStyles(),
        ]);
    ...
    public function edit(Request $request, ExternalDebt $debt)
    {
        ...
        return Inertia::render('Debts/ExternalPayables/Edit', [
            'item' => $debt,
            'filters' => $filters,
            'companies' => Company::orderBy('name', 'asc')->get(),
            'branches' => Branch::whereHas('branchGroup', function ($q) use ($companyId) {
    ...
    public function update(Request $request, ExternalDebt $debt)
    {
        ...
        return redirect()->route('external-payables.show', $debt->id)
            ->with('success', 'Hutang eksternal berhasil diubah.');
    }
    ...
    public function destroy(Request $request, ExternalDebt $debt)
    {
        ...
        return redirect()->route('external-payables.index')
            ->with('success', 'Hutang eksternal berhasil dihapus.');
    }
