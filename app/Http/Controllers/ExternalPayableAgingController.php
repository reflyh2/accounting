...
class ExternalPayableAgingController extends Controller
{
   public function index(Request $request)
   {
      ...
      return Inertia::render('Reports/ExternalPayableAging', [
         'companies' => $companies,
         'branches' => $branches,
         'filters' => $filters,
   ...
   private function getReportData(array $filters): array
   {
      ...
      // Load partners and build rows (only those with positive total)
   ...
   {
      $pdf = Pdf::loadView('reports.external-payable-aging', [
         'reportData' => $reportData,
         'filters' => $filters,
      ]);
      return $pdf->download('umur-hutang-luar-' . date('Y-m-d') . '.pdf');
   }
   ...
   private function downloadExcel(array $reportData, array $filters)
   {
      ...
      // Title
      $sheet->setCellValue('A1', 'Umur Hutang Luar');
      ...
      $filename = 'umur-hutang-luar-' . date('Y-m-d') . '.xlsx';
