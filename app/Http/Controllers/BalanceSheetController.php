...
class BalanceSheetController extends Controller
{
    ...
    {
        return $accounts->map(function ($account) use ($currentBalances, $previousBalances, $isDetailed) {
            return $this->mapAccountBalances($account, $currentBalances, $previousBalances, $isDetailed);
        })->filter(function ($item) {
            return $item['balances']['current'] != 0 || $item['balances']['previous'] != 0;
        })->values();
    }
