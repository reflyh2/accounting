...
class ExternalPayableMutationController extends Controller
{
  public function index(Request $request)
  {
    ...
    return Inertia::render('Reports/ExternalPayableMutation', [
      'companies' => $companies,
      'branches' => $branches,
      'filters' => $filters,
  ...
  private function getReportData(array $filters): array
  {
    ...
    // Opening: debts up to openingCutoff
    ...
    // Opening payments up to openingCutoff
    ...
    // Payments in range (effective date logic)
  ...
  {
    $pdf = Pdf::loadView('reports.external-payable-mutation', [
      'reportData' => $reportData,
      'filters' => $filters,
    ]);
    return $pdf->download('mutasi-hutang-luar-' . date('Y-m-d') . '.pdf');
  }
  ...
  private function downloadExcel(array $reportData, array $filters)
  {
    ...
    // Title
    $sheet->setCellValue('A1', 'Mutasi Hutang Luar');
    ...
    $filename = 'mutasi-hutang-luar-' . date('Y-m-d') . '.xlsx';
