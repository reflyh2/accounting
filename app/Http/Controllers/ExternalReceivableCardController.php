...
class ExternalReceivableCardController extends Controller
{
   public function index(Request $request)
   {
      ...
      return Inertia::render('Reports/ExternalReceivableCard', [
         'companies' => $companies,
         'branches' => $branches,
         'partners' => $partners,
         'filters' => $filters,
   ...
   {
      $pdf = Pdf::loadView('reports.external-receivable-card', [
         'cardData' => $cardData,
         'filters' => $filters,
      ]);
      return $pdf->download('kartu-piutang-' . date('Y-m-d') . '.pdf');
   }
   ...
   private function downloadExcel(array $cardData, array $filters)
   {
      ...
      // Title
      $sheet->setCellValue('A1', 'Kartu Piutang');
      ...
      $filename = 'kartu-piutang-' . date('Y-m-d') . '.xlsx';
