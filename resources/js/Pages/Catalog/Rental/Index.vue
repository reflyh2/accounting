...
<template>
    <Head title="Catalog - Rental" />
    <AuthenticatedLayout>
        <template #header>
            <h2>Catalog: Rental</h2>
        </template>

        <div class="bg-white shadow-sm sm:rounded border border-gray-200 p-4">
            <div class="mb-3 flex justify-between">
                <div></div>
                <Link :href="route('catalog.rental.create')" class="px-3 py-1.5 bg-main-600 hover:bg-main-700 text-white rounded">
                    New Rental
                </Link>
            </div>
                ...
                <tbody>
                    <tr v-for="item in items.data" :key="item.id">
                        ...
                        <td class="border px-2 py-1.5 text-right">
                            <Link :href="route('catalog.rental.edit', item.id)" class="text-main-600 hover:underline">Edit</Link>
                        </td>
                    </tr>
                </tbody>
            </table>
