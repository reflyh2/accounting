...
function submit() {
    if (props.mode === 'edit') {
        form.put(route('catalog.packages.update', props.product.id));
    } else {
        form.post(route('catalog.packages.store'));
    }
}
...
<template>
    <Head :title="props.mode === 'edit' ? 'Edit Package' : 'Create Package'" />
    <AuthenticatedLayout>
        <template #header>
            <h2>{{ props.mode === 'edit' ? 'Edit Package' : 'Create Package' }}</h2>
        </template>
        ...
        <div class="bg-white shadow-sm sm:rounded border border-gray-200 p-4">
            <form @submit.prevent="submit" class="space-y-4 max-w-2xl">
                ...
                <div class="flex items-center mt-2">
                    <input id="is_active_pkg" v-model="form.is_active" type="checkbox" class="mr-2">
                    <label for="is_active_pkg">Active</label>
                </div>
                ...
                <div class="flex items-center">
                    ...
                    <AppSecondaryButton :href="route('catalog.packages.index')" as="a">Cancel</AppSecondaryButton>
                </div>
            </form>
        </div>
    </AuthenticatedLayout>
