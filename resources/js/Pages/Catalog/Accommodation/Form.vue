...
function submit() {
    if (props.mode === 'edit') {
        form.put(route('catalog.accommodation.update', props.product.id));
    } else {
...
<template>
    <Head :title="props.mode === 'edit' ? 'Edit Accommodation' : 'Create Accommodation'" />
    <AuthenticatedLayout>
        <template #header>
            <h2>{{ props.mode === 'edit' ? 'Edit Accommodation' : 'Create Accommodation' }}</h2>
        </template>
        ...
        <div class="bg-white shadow-sm sm:rounded border border-gray-200 p-4">
            <form @submit.prevent="submit" class="space-y-4 max-w-2xl">
                ...
                <div class="flex items-center">
                    ...
                    <AppSecondaryButton :href="route('catalog.accommodation.index')" as="a">Cancel</AppSecondaryButton>
                </div>
            </form>
        </div>
    </AuthenticatedLayout>
