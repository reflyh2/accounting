...
const form = useForm({
   company_id: props.cashReceiptJournal?.branch?.branch_group?.company_id || null,
   branch_id: props.cashReceiptJournal?.branch_id || (props.branches.length > 1 ? null : (props.branches[0]?.id || null)),
   date: props.cashReceiptJournal?.date || new Date().toISOString().split('T')[0],
   reference_number: props.cashReceiptJournal?.reference_number || '',
   description: props.cashReceiptJournal?.description || '',
   kas_bank_account_id: props.cashReceiptJournal?.journal_entries?.find(entry => entry.debit != 0)?.account_id || null,
   kas_bank_account_currency_id: props.cashReceiptJournal?.journal_entries?.find(entry => entry.debit != 0)?.currency_id || null,
   kas_bank_account_exchange_rate: props.cashReceiptJournal?.journal_entries?.find(entry => entry.debit != 0)?.exchange_rate || 1,
   entries: props.cashReceiptJournal?.journal_entries?.filter(entry => entry.debit == 0) || [
      { account_id: null, credit: 0, currency_id: null, exchange_rate: 1 },
   ],
   create_another: false,
});

const kas_bank_account_amount = computed(() => {
   return form.entries.reduce((sum, entry) => sum + (Number(entry.credit) * Number(entry.exchange_rate)), 0) / form.kas_bank_account_exchange_rate;
});

const submitted = ref(false);
const selectedCompany = ref(props.cashReceiptJournal?.branch?.branch_group.company_id || (props.companies.length > 1 ? null : props.companies[0].id));
const minDate = ref(props.cashReceiptJournal ? new Date(new Date(props.cashReceiptJournal.date).getFullYear(), 0, 1).toLocaleDateString('en-CA') : null);
const maxDate = ref(props.cashReceiptJournal ? new Date(new Date(props.cashReceiptJournal.date).getFullYear(), 11, 31).toLocaleDateString('en-CA') : null);
...
watch(
    () => props.branches,
    (newBranches) => {
        if (!props.cashReceiptJournal && newBranches.length === 1) {
...
onMounted(() => {
   selectedCompany.value = props.cashReceiptJournal?.branch?.branch_group.company_id || (props.companies.length > 1 ? null : props.companies[0].id);
});

function addEntry() {
   form.entries.push({ account_id: null, credit: 0, currency_id: null, exchange_rate: 1 });
}
...
function submitForm(createAnother = false) {
   ...
   if (props.cashReceiptJournal) {
      form.put(route('cash-receipt-journals.update', props.cashReceiptJournal.id), {
         preserveScroll: true,
      });
   } else {
      form.post(route('cash-receipt-journals.store'), {
         preserveScroll: true,
      });
   }
}
   ...
   <form @submit.prevent="submitForm(false)" class="space-y-4">
         ...
         <div class="w-2/3 max-w-2xl mr-8">
            <div class="grid grid-cols-2 gap-4">
               <AppSelect
                  ...
                  :disabled="!!props.cashReceiptJournal"
                  required
               />
               <AppSelect
         ...
         <table class="min-w-full bg-white border border-gray-300">
            ...
            <tfoot>
               <tr class="text-sm">
                  ...
                  <th class="border border-gray-300 px-1.5 py-1.5 text-left">{{ props.primaryCurrency.symbol + ' ' + formatNumber(form.entries.reduce((sum, entry) => sum + (Number(entry.credit) * Number(entry.exchange_rate)), 0)) }}</th>
                  <th colspan="3"></th>
               </tr>
            </tfoot>
         </table>
      ...
      <div class="mt-4 flex items-center">
         <AppPrimaryButton type="submit" class="mr-2">
            {{ props.cashReceiptJournal ? 'Ubah' : 'Tambah' }} Penerimaan Kas
         </AppPrimaryButton>
         <AppUtilityButton v-if="!props.cashReceiptJournal" type="button" @click="submitForm(true)" class="mr-2">
            Tambah & Buat Lagi
         </AppUtilityButton>
         <AppSecondaryButton @click="$inertia.visit(route('cash-receipt-journals.index', filters))">
            Batal
         </AppSecondaryButton>
      </div>
   </form>
