...
const form = useForm({
    company_id: props.assetSale?.branch?.branch_group?.company_id || null,
    branch_id: props.assetSale?.branch_id || null,
    partner_id: props.assetSale?.partner_id || null,
    currency_id: props.assetSale?.currency_id || page.props.primaryCurrency?.id || null,
    exchange_rate: props.assetSale?.exchange_rate || 1,
    invoice_date: props.assetSale?.invoice_date || new Date().toISOString().split('T')[0],
    due_date: props.assetSale?.due_date || new Date().toISOString().split('T')[0],
    notes: props.assetSale?.notes || '',
    status: props.assetSale?.status || 'open',
    details: props.assetSale?.asset_invoice_details?.map(detail => ({
        id: detail.id,
...
watch(
    () => props.branches,
    (newBranches) => {
        if (!props.assetSale && newBranches && newBranches.length === 1) {
...
onMounted(() => {
    selectedCompany.value = props.assetSale?.branch?.branch_group.company_id || (props.companies.length > 1 ? null : props.companies[0]?.id);
    if (!props.assetSale && props.branches && props.branches.length === 1) {
...
function submitForm(createAnother = false) {
    ...
    if (props.assetSale) {
        form.put(route('asset-sales.update', props.assetSale.id), {
            preserveScroll: true,
            onSuccess: () => { submitted.value = false; },
            onError: () => { submitted.value = false; }
        });
    } else {
        form.post(route('asset-sales.store'), {
            preserveScroll: true,
            onSuccess: () => {
    ...
    <form @submit.prevent="submitForm(false)" class="space-y-4">
        <div class="flex justify-between">
            ...
            <div class="w-1/3 bg-gray-100 p-4 rounded-lg text-sm">
                <h3 class="text-lg font-semibold mb-2">Informasi Penjualan Aset</h3>
                <p class="mb-2">Catat detail penjualan aset kepada customer. Pastikan semua informasi akurat.</p>
                <ul class="list-disc list-inside">
                    <li>Pilih Perusahaan dan Cabang terkait.</li>
                    <li>Pilih Customer yang akan membeli aset.</li>
                    ...
                    <li>Isi detail aset yang dijual pada tabel di bawah.</li>
                </ul>
            </div>
        </div>

        <div class="overflow-x-auto mt-6">
            <h3 class="text-lg font-semibold mb-2">Detail Penjualan Aset</h3>
            ...
            <table class="min-w-full bg-white border border-gray-300">
                <thead>
                    <tr class="bg-gray-100">
                        ...
                        <th class="border border-gray-300 text-sm min-w-36 px-1.5 py-1.5">Harga Jual</th>
        ...
        <div class="mt-6 flex items-center">
            <AppPrimaryButton type="submit" :disabled="form.processing || submitted" class="mr-2">
                {{ props.assetSale ? 'Ubah' : 'Simpan' }} Faktur
            </AppPrimaryButton>
            <AppUtilityButton v-if="!props.assetSale" type="button" @click="submitForm(true)" :disabled="form.processing || submitted" class="mr-2">
                Simpan & Buat Lagi
            </AppUtilityButton>
            <AppSecondaryButton @click="$inertia.visit(route('asset-sales.index', filters))" :disabled="form.processing || submitted">
                Batal
            </AppSecondaryButton>
        </div>
    </form>
