            ...
            <div class="bg-white shadow-sm sm:rounded border border-gray-200">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="grid grid-cols-4 gap-4 mb-4">
                        ...
                        <AppSelect v-model="form.transaction_type" :options="transactionTypes" label="Tipe Transaksi"
                    ...
                    <div class="overflow-x-auto">
                        <ReportTable>
                            ...
                            <tbody>
                                <tr v-for="item in data.data" :key="item.product_variant_id"
                                    ...
                                    <ReportTD class="text-right font-medium">{{ formatCurrency(item.begin_value) }}
                                    </ReportTD>
                                    <!-- In -->
                                    <ReportTD class="text-right bg-green-50/30 text-green-700">{{
                                        formatNumber(item.qty_in) }}
                                    </ReportTD>
                                    <ReportTD class="text-right bg-green-50/30 text-green-700">{{
                                        formatUnitCost(item.avg_cost_in) }}</ReportTD>
                                    <ReportTD class="text-right bg-green-50/30 text-green-700 font-medium">{{
                                        formatCurrency(item.value_in) }}</ReportTD>
                                    <!-- Out -->
                                    <ReportTD class="text-right bg-red-50/30 text-red-700">{{ formatNumber(item.qty_out)
                                    }}
                                    </ReportTD>
                                    <ReportTD class="text-right bg-red-50/30 text-red-700">{{
                                        formatUnitCost(item.avg_cost_out)
                                    }}</ReportTD>
                                    <ReportTD class="text-right bg-red-50/30 text-red-700 font-medium">{{
                                        formatCurrency(item.value_out) }}</ReportTD>
                                    <!-- Ending -->
                                    <ReportTD class="text-right bg-blue-50/30 font-medium">{{ formatNumber(item.end_qty)
                                    }}
                                    </ReportTD>
                                    <ReportTD class="text-right bg-blue-50/30">{{ formatUnitCost(item.end_avg_cost) }}
                                    </ReportTD>
                                    <ReportTD class="text-right bg-blue-50/30 font-bold">{{
                                        formatCurrency(item.end_value) }}
                                    </ReportTD>
                                </tr>
                            ...
                            <tfoot v-if="data.data?.length && totals">
                                <tr class="group bg-gray-100 font-bold">
                                    ...
                                    <ReportTD class="text-right text-green-700">{{ formatCurrency(totals.value_in) }}
                                    </ReportTD>
                                    <ReportTD :colspan="2"></ReportTD>
                                    <ReportTD class="text-right text-red-700">{{ formatCurrency(totals.value_out) }}
                                    </ReportTD>
                                    <ReportTD :colspan="2"></ReportTD>
