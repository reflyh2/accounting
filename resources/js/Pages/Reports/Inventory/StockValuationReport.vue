            ...
            <div class="bg-white shadow-sm sm:rounded border border-gray-200">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="grid grid-cols-4 gap-4 mb-4">
                        <AppSelect v-model="form.company_id"
                            :options="companies.map(c => ({ value: c.id, label: c.name }))" label="Perusahaan" multiple
                            placeholder="Semua Perusahaan" />
                        <AppSelect v-model="form.branch_id"
                            :options="branches.map(b => ({ value: b.id, label: b.name }))" label="Cabang" multiple
                            placeholder="Semua Cabang" />
                        <AppSelect v-model="form.location_id"
                            :options="[{ value: '', label: 'Semua Lokasi' }, ...locations.map(l => ({ value: l.id, label: l.name }))]"
                            label="Lokasi" placeholder="Semua Lokasi" />
                        <AppSelect v-model="form.category_id"
                            :options="[{ value: '', label: 'Semua Kategori' }, ...categories.map(c => ({ value: c.id, label: c.name }))]"
                            label="Kategori" placeholder="Semua Kategori" />
                    </div>
                    ...
                    <div v-if="totals" class="mb-4 p-4 bg-green-50 rounded-lg flex gap-8 flex-wrap">
                        <div>
                            ...
                            <span class="ml-2 font-bold text-green-900">{{ formatNumber(totals.distinct_products, 0)
                            }}</span>
                        </div>
                        <div>
                            <span class="text-sm text-green-600">Total On Hand:</span>
                            <span class="ml-2 font-bold text-green-900">{{ formatNumber(totals.total_qty_on_hand)
                            }}</span>
                        </div>
                        <div>
                            <span class="text-sm text-green-600">Total Reserved:</span>
                            <span class="ml-2 font-bold text-green-900">{{ formatNumber(totals.total_qty_reserved)
                            }}</span>
                        </div>
                        <div>
                            <span class="text-sm text-green-600">Total Available:</span>
                            <span class="ml-2 font-bold text-green-900">{{ formatNumber(totals.total_qty_available)
                            }}</span>
                        </div>
                        <div>
                            <span class="text-sm text-green-600">Total Nilai On Hand:</span>
                            <span class="ml-2 font-bold text-green-900">{{ formatCurrency(totals.total_value_on_hand)
                            }}</span>
                        </div>
                    </div>
                        ...
                        <ReportTable>
                            ...
                            <tbody>
                                <tr v-for="item in data.data" :key="item.id" class="group hover:bg-gray-50">
                                    <ReportTD>
                                        {{ item.product_name }}
                                        <span v-if="item.variant_name !== '-'" class="text-gray-400"> - {{
                                            item.variant_name
                                        }}</span>
                                    </ReportTD>
                                    ...
                                    <ReportTD class="text-right bg-blue-50/30">{{ formatNumber(item.qty_on_hand) }}
                                    </ReportTD>
                                    <ReportTD class="text-right bg-blue-50/30 font-medium">{{
                                        formatCurrency(item.value_on_hand)
                                    }}</ReportTD>
                                    <ReportTD class="text-right bg-amber-50/30">{{ formatNumber(item.qty_reserved) }}
                                    </ReportTD>
                                    <ReportTD class="text-right bg-amber-50/30 font-medium">{{
                                        formatCurrency(item.value_reserved) }}</ReportTD>
                                    <ReportTD class="text-right bg-green-50/30">{{ formatNumber(item.qty_available) }}
                                    </ReportTD>
                                    <ReportTD class="text-right bg-green-50/30 font-medium">{{
                                        formatCurrency(item.value_available) }}</ReportTD>
                                </tr>
                            ...
                            <tfoot v-if="data.data?.length">
                                <tr class="group bg-gray-100 font-bold">
                                    ...
                                    <ReportTD class="text-right">{{ formatCurrency(totals.total_value_on_hand) }}
                                    </ReportTD>
                                    <ReportTD class="text-right">{{ formatNumber(totals.total_qty_reserved) }}
                                    </ReportTD>
                                    <ReportTD class="text-right">{{ formatCurrency(totals.total_value_reserved) }}
                                    </ReportTD>
                                    <ReportTD class="text-right">{{ formatNumber(totals.total_qty_available) }}
                                    </ReportTD>
                                    <ReportTD class="text-right">{{ formatCurrency(totals.total_value_available) }}
                                    </ReportTD>
                                </tr>
                            </tfoot>
                        </ReportTable>
