...
function generateReport() {
    router.get(route('sales-reports.deliveries'), form.value, {
        preserveState: false,
        preserveScroll: true,
    });
}
...
<template>
    <Head title="Laporan Pengiriman Barang" />
    ...
    <AuthenticatedLayout>
        ...
        <div class="mx-auto">
            <SalesReportTabs activeTab="sales-reports.deliveries" />
            ...
            <div class="bg-white shadow-sm sm:rounded border border-gray-200">
                <div class="p-6 text-gray-900">
                    ...
                    <div v-if="totals" class="mb-4 p-4 bg-blue-50 rounded-lg flex gap-8">
                        <div>
                            <span class="text-sm text-blue-600">Total Pengiriman:</span>
                            <span class="ml-2 font-bold text-blue-900">{{ formatNumber(totals.count) }}</span>
                        </div>
                        <div>
                            <span class="text-sm text-blue-600">Nilai Total:</span>
                            <span class="ml-2 font-bold text-blue-900">{{ formatCurrency(totals.total_amount) }}</span>
                        </div>
                    </div>
                            ...
                            <div v-for="(group, index) in data" :key="index" class="border rounded-lg overflow-hidden">
                                ...
                                <table class="min-w-full divide-y divide-gray-200">
                                    <thead class="bg-gray-50">
                                        <tr>
                                            <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">No. Delivery</th>
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        <tr v-for="(item, idx) in group.items" :key="idx" class="hover:bg-gray-50">
                                            <td class="px-4 py-2 text-sm">
                                                <Link :href="route('sales-deliveries.show', item.document.id)" class="text-blue-600 hover:underline">
                                                    {{ item.document.delivery_number }}
                                                </Link>
                                            </td>
                                            <td class="px-4 py-2 text-sm">{{ formatDate(item.document.delivery_date) }}</td>
                                ...
                                <table class="min-w-full divide-y divide-gray-200">
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        <tr v-for="item in group.items" :key="item.id" class="hover:bg-gray-50">
                                            <td class="px-4 py-2 text-sm">
                                                <Link :href="route('sales-deliveries.show', item.id)" class="text-blue-600 hover:underline">
                                                    {{ item.delivery_number }}
                                                </Link>
                                            </td>
                                            <td class="px-4 py-2 text-sm">{{ formatDate(item.delivery_date) }}</td>
                    ...
                    <template v-else>
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-gray-200">
                                <thead class="bg-gray-50">
                                    <tr>
                                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">No. Delivery</th>
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    <tr v-for="item in data.data" :key="item.id" class="hover:bg-gray-50">
                                        ...
                                        <td class="px-4 py-3 text-sm">{{ formatDate(item.delivery_date) }}</td>
