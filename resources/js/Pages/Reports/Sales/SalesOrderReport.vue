...
// Use props.filters.group_by for display conditions (what data we received)
// Use form.group_by for the form input (what user selected)
...
function generateReport() {
    router.get(route('sales-reports.orders'), form.value, {
        preserveState: false,
        preserveScroll: true,
    });
}
...
<template>
    <Head title="Laporan Sales Order" />
    ...
    <AuthenticatedLayout>
        ...
        <div class="mx-auto">
            <SalesReportTabs activeTab="sales-reports.orders" />
            ...
            <div class="bg-white shadow-sm sm:rounded border border-gray-200">
                <div class="p-6 text-gray-900">
                    ...
                    <div v-if="totals" class="mb-4 p-4 bg-green-50 rounded-lg flex gap-8">
                        <div>
                            <span class="text-sm text-green-600">Total SO:</span>
                        ...
                        <div>
                            ...
                            <span class="ml-2 font-bold text-green-900">{{ formatCurrency(totals.total_amount) }}</span>
                        </div>
                    </div>
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        <tr v-for="(item, idx) in group.items" :key="idx" class="hover:bg-gray-50">
                                            <td class="px-4 py-2 text-sm">
                                                <Link :href="route('sales-orders.show', item.document.id)" class="text-green-600 hover:underline">
                                                    {{ item.document.order_number }}
                                                </Link>
                                            </td>
                                            <td class="px-4 py-2 text-sm">{{ formatDate(item.document.order_date) }}</td>
                                ...
                                <table class="min-w-full divide-y divide-gray-200">
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        <tr v-for="item in group.items" :key="item.id" class="hover:bg-gray-50">
                                            <td class="px-4 py-2 text-sm">
                                                <Link :href="route('sales-orders.show', item.id)" class="text-green-600 hover:underline">
                                                    {{ item.order_number }}
                                                </Link>
                                            </td>
