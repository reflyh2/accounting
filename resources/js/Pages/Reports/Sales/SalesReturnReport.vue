...
function generateReport() {
    router.get(route('sales-reports.returns'), form.value, {
        preserveState: false,
        preserveScroll: true,
    });
}
...
<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="mx-auto">
            <SalesReportTabs activeTab="sales-reports.returns" />
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        <tr v-for="(item, idx) in group.items" :key="idx" class="hover:bg-gray-50">
                                            <td class="px-4 py-2 text-sm">
                                                <Link :href="route('sales-returns.show', item.document.id)" class="text-orange-600 hover:underline">
                                ...
                                <table class="min-w-full divide-y divide-gray-200">
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        <tr v-for="item in group.items" :key="item.id" class="hover:bg-gray-50">
                                            <td class="px-4 py-2 text-sm">
                                                <Link :href="route('sales-returns.show', item.id)" class="text-orange-600 hover:underline">
                                                    {{ item.return_number }}
                                                </Link>
                                            </td>
                                            <td class="px-4 py-2 text-sm">{{ formatDate(item.return_date) }}</td>
                                            <td class="px-4 py-2 text-sm">{{ item.sales_delivery?.delivery_number || '-' }}</td>
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    <tr v-for="item in data.data" :key="item.id" class="hover:bg-gray-50">
                                        ...
                                        <td class="px-4 py-3 text-sm">{{ item.sales_delivery?.delivery_number || '-' }}</td>
