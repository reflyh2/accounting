...
function generateReport() {
    router.get(route('purchasing-reports.receipts'), form.value, {
        preserveState: false,
        preserveScroll: true,
    });
}
...
<template>
    <Head title="Laporan Penerimaan Barang" />
    ...
    <AuthenticatedLayout>
        ...
        <div class="mx-auto">
            <PurchasingReportTabs activeTab="purchasing-reports.receipts" />
            ...
            <div class="bg-white shadow-sm sm:rounded border border-gray-200">
                <div class="p-6 text-gray-900">
                    ...
                    <!-- Summary -->
                    <div v-if="totals" class="mb-4 p-4 bg-green-50 rounded-lg flex gap-8">
                        <div>
                            <span class="text-sm text-green-600">Total GRN:</span>
                            <span class="ml-2 font-bold text-green-900">{{ formatNumber(totals.count) }}</span>
                        </div>
                        <div>
                            <span class="text-sm text-green-600">Nilai Total:</span>
                            <span class="ml-2 font-bold text-green-900">{{ formatCurrency(totals.total_value) }}</span>
                        </div>
                    </div>
                            ...
                            <div v-for="(group, index) in data" :key="index" class="border rounded-lg overflow-hidden">
                                ...
                                <table class="min-w-full divide-y divide-gray-200">
                                    <thead class="bg-gray-50">
                                        <tr>
                                            <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">No. GRN</th>
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        <tr v-for="(item, idx) in group.items" :key="idx" class="hover:bg-gray-50">
                                            <td class="px-4 py-2 text-sm">
                                                <Link :href="route('goods-receipts.show', item.document.id)" class="text-blue-600 hover:underline">
                                                    {{ item.document.receipt_number }}
                                                </Link>
                                            </td>
                                            <td class="px-4 py-2 text-sm">{{ formatDate(item.document.receipt_date) }}</td>
                                ...
                                <table class="min-w-full divide-y divide-gray-200">
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        <tr v-for="item in group.items" :key="item.id" class="hover:bg-gray-50">
                                            <td class="px-4 py-2 text-sm">
                                                <Link :href="route('goods-receipts.show', item.id)" class="text-blue-600 hover:underline">
                                                    {{ item.receipt_number }}
                                                </Link>
                                            </td>
                                            <td class="px-4 py-2 text-sm">{{ formatDate(item.receipt_date) }}</td>
                            ...
                            <table class="min-w-full divide-y divide-gray-200">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    <tr v-for="item in data.data" :key="item.id" class="hover:bg-gray-50">
                                        ...
                                        <td class="px-4 py-3 text-sm">{{ formatDate(item.receipt_date) }}</td>
                                        <td class="px-4 py-3 text-sm">{{ item.supplier?.name || '-' }}</td>
