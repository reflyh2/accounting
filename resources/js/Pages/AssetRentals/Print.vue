...
<template>
    <Head :title="`Cetak Faktur ${assetRental.number}`" />
    <div class="p-6 bg-white text-gray-900 print:p-0 print:m-0 print:text-black print:bg-white">
        <h2 class="text-2xl font-bold mb-4 text-center">Faktur Penyewaan Aset</h2>
        <h3 class="text-lg font-semibold mb-6 text-center">#{{ assetRental.number }}</h3>
            ...
            <table class="w-full border-collapse border border-gray-400 text-xs">
                <thead>
                    <tr class="bg-gray-100 print:bg-gray-100">
                        ...
                        <th class="border border-gray-400 px-1 py-1 text-right" colspan="2">Harga Satuan</th>
                        <th class="border border-gray-400 px-1 py-1 text-center">Mulai Sewa</th>
                        <th class="border border-gray-400 px-1 py-1 text-center">Akhir Sewa</th>
                ...
                <tbody>
                    <tr v-for="detail in assetRental.asset_invoice_details" :key="detail.id">
                        ...
                        <td class="border border-r-0 border-gray-400 px-1 py-1 text-right">{{ assetRental.currency?.symbol }}</td>
                        <td class="border border-l-0 border-gray-400 px-1 py-1 text-right">{{ formatNumber(detail.unit_price) }}</td>
                        <td class="border border-gray-400 px-1 py-1 text-center">{{ detail.rental_start_date ? new Date(detail.rental_start_date).toLocaleDateString('id-ID') : '-' }}</td>
                        <td class="border border-gray-400 px-1 py-1 text-center">{{ detail.rental_end_date ? new Date(detail.rental_end_date).toLocaleDateString('id-ID') : '-' }}</td>
                        <td class="border border-r-0 border-gray-400 px-1 py-1 text-right">{{ assetRental.currency?.symbol }}</td>
                        <td class="border border-l-0 border-gray-400 px-1 py-1 text-right">{{ formatNumber(detail.line_amount) }}</td>
                    </tr>
                    <tr v-if="!assetRental.asset_invoice_details || assetRental.asset_invoice_details.length === 0">
                        <td colspan="8" class="border border-gray-400 px-1 py-1 text-center text-gray-500">Tidak ada detail item.</td>
                    </tr>
                </tbody>
                <tfoot>
                    <tr class="bg-gray-100 print:bg-gray-100">
                        <td colspan="8" class="border border-gray-400 px-1 py-1 text-right font-semibold">Total Faktur</td>
                        <td class="border border-r-0 border-gray-400 px-1 py-1 text-right font-semibold">{{ assetRental.currency?.symbol }}</td>
        ...
        <div class="mt-12 text-xs grid grid-cols-3 gap-4">
            <div class="text-center">
                <p>Dibuat Oleh,</p>
                <div class="mt-12 border-t border-gray-400 pt-1">{{ assetRental.creator?.name ?? '(____________________)' }}</div>
            </div>
             <div class="text-center">
                <p>Disetujui Oleh,</p>
            ...
            <div class="text-center">
                <p>Diterima Oleh,</p>
                 <div class="mt-12 border-t border-gray-400 pt-1">{{ assetRental.partner?.name ?? '(____________________)' }}</div>
            </div>
        </div>
    </div>
</template>
