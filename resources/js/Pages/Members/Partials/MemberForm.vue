...
const form = useForm({
    ...
    tax_id: props.member?.tax_id || '',
    registration_number: props.member?.registration_number || '',
    industry: props.member?.industry || '',
    website: props.member?.website || '',
    status: props.member?.status || 'active',
    company_ids: props.member?.companies?.map(company => company.id) || [],
    credit_terms: props.member?.credit_terms || {
        credit_limit: 0,
        used_credit: 0,
        payment_term_days: 0,
    ...
    tags: props.member?.tags?.map(tag => tag.tag_name) || [],
    custom_fields: props.member?.custom_fields || [],
...
<template>
    <form @submit.prevent="submitForm(false)" class="max-w-7xl mx-auto">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-4">
                ...
                <AppInput
                    ...
                    type="number"
                    label="Batas Kredit:"
                    placeholder="Masukkan batas kredit"
                ...
                <AppInput
                    ...
                    placeholder="Masukkan kredit yang digunakan"
        ...
        <div class="mt-6 flex items-center justify-start gap-2">
            ...
            <AppSecondaryButton 
                type="button"
                @click="$inertia.visit(route('members.index', filters))"
            >
                Batal
            </AppSecondaryButton>
        </div>
