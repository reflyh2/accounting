...
const deleteItem = () => {
    form.delete(route('external-receivables.destroy', props.item.id), { onSuccess: () => { showDeleteConfirmation.value = false; } });
};
        ...
        <div class="min-w-max sm:min-w-min md:max-w-full mx-auto">
            <div class="bg-white overflow-auto shadow-sm sm:rounded-s border-y border-l border-gray-200">
                <div class="p-6 text-gray-900">
                    <div class="mb-6">
                        <AppBackLink :href="route('external-receivables.index', filters)" text="Kembali ke Daftar Piutang Eksternal" />
                    </div>

                    <div class="flex justify-between items-center mb-4">
                        <h3 class="text-lg font-bold">Nomor #{{ item.number }}</h3>
                        <div class="flex items-center">
                            <a :href="route('external-receivables.edit', item.id)" class="mr-1">
