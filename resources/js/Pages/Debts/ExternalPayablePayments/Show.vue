...
const deleteItem = () => {
    form.delete(route('external-payable-payments.destroy', props.item.id), {
...
<template>
    <Head title="Detail Pembayaran Hutang" />
    <AuthenticatedLayout>
        <template #header>
            ...
            <div class="mx-auto">
                <div class="bg-white overflow-auto shadow-sm sm:rounded-s border-y border-l border-gray-200">
                    <div class="p-6 text-gray-900">
                        <div class="mb-6">
                            <AppBackLink :href="route('external-payable-payments.index', filters)" text="Kembali ke Pembayaran Hutang" />
                        </div>

                        <div class="flex justify-between items-center mb-4">
                            <h3 class="text-lg font-bold">Dokumen #{{ item.number }}</h3>
                            <div class="flex items-center">
                                <a :href="route('external-payable-payments.print', item.id)" target="_blank" class="mr-1">
                                    <AppPrintButton title="Print" />
                                </a>
                                <a :href="route('external-payable-payments.edit', item.id)" class="mr-1">
                        ...
                        <div class="mt-6">
                            <h4 class="text-md font-semibold mb-2">Detail Pembayaran</h4>
                            <table class="w-full border-collapse border border-gray-300 text-sm">
                                <thead>
                                    <tr class="bg-gray-50">
                                        <th class="border border-gray-300 px-1.5 py-1.5 text-left"># Hutang</th>
                                        <th class="border border-gray-300 px-1.5 py-1.5 text-left">Tanggal Terbit</th>
                                        <th class="border border-gray-300 px-1.5 py-1.5 text-right">Dibayar</th>
                                    </tr>
                                </thead>
                                <tbody>
        ...
        <DeleteConfirmationModal
            ...
            message="Apakah Anda yakin ingin menghapus pembayaran hutang ini? Tindakan ini tidak dapat dibatalkan."
