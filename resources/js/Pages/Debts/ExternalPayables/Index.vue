...
function deleteItem(id) {
    router.delete(route('external-payables.destroy', id), { preserveScroll: true, preserveState: true });
}

function handleBulkDelete(ids) {
    // You may add bulk delete route later if needed
}

function handleSort(newSort) {
    currentSort.value = newSort;
    router.get(route('external-payables.index'), { ...route().params, ...currentFilters.value, sort: newSort.key, order: newSort.order, per_page: props.perPage }, { preserveState: true, preserveScroll: true, replace: true });
}

function handleFilter(newFilters) {
    currentFilters.value = newFilters;
    if (newFilters.page) delete newFilters.page;
    router.get(route('external-payables.index'), { ...currentFilters.value, sort: currentSort.value.key, order: currentSort.value.order, per_page: newFilters.per_page || props.perPage, page: 1 }, { preserveState: true, preserveScroll: true, replace: true });
}
            ...
            <div class="bg-white shadow-sm sm:rounded border border-gray-200">
                <div class="text-gray-900">
                    <AppDataTable
                        ...
                        :createRoute="{ name: 'external-payables.create' }"
                        :editRoute="{ name: 'external-payables.edit' }"
                        :deleteRoute="{ name: 'external-payables.destroy' }"
                        :viewRoute="{ name: 'external-payables.show' }"
                        :indexRoute="{ name: 'external-payables.index' }"
                        :downloadOptions="downloadOptions"
                        :sortable="sortableColumns"
                        :defaultSort="defaultSort"
                        :currentSort="currentSort"
                        :perPage="perPage"
                        routeName="external-payables.index"
                        itemKey="id"
                        importRoute="external-payables.import"
                        importTemplateRoute="external-payables.import-template"
                        importTitle="Impor Saldo Awal Hutang"
                        importDescription="Unggah berkas CSV atau Excel. Tiap baris membuat satu hutang dan otomatis mencatat jurnal lawan ke akun yang dipilih (mis. Modal Awal)."
                        importButtonLabel="Impor Saldo Awal"
