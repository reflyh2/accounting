...
const form = useForm({
    ...
    company_ids: props.customer?.companies?.map(company => company.id) || [],
    credit_terms: props.customer?.credit_terms || {
        credit_limit: 0,
        used_credit: 0,
        payment_term_days: 0,
...
<template>
    <form @submit.prevent="submitForm(false)" class="max-w-7xl mx-auto">
        ...
        <div class="mt-6 flex items-center justify-start gap-2">
            ...
            <AppSecondaryButton 
                type="button"
                @click="$inertia.visit(route('customers.index', filters))"
            >
                Batal
            </AppSecondaryButton>
        </div>
