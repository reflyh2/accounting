...
const form = useForm({
    ...
    company_ids: props.supplier?.companies?.map(company => company.id) || [],
    credit_terms: props.supplier?.credit_terms || {
        credit_limit: 0,
        used_credit: 0,
        payment_term_days: 0,
    ...
    tags: props.supplier?.tags?.map(tag => tag.tag_name) || [],
    custom_fields: props.supplier?.custom_fields || [],
...
<template>
    <form @submit.prevent="submitForm(false)" class="max-w-7xl mx-auto">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-4">
                ...
                <AppInput
                    ...
                    type="number"
                    label="Hari Term Pembayaran:"
                    placeholder="Masukkan hari term pembayaran"
        ...
        <div class="mt-6 flex items-center justify-start gap-2">
            ...
            <AppSecondaryButton 
                type="button"
                @click="$inertia.visit(route('suppliers.index', filters))"
            >
                Batal
            </AppSecondaryButton>
        </div>
