...
<template>
    <Head :title="isAmortization ? 'Tambah Entri Amortisasi' : 'Tambah Entri Penyusutan'" />

    <AuthenticatedLayout>
        <template #header>
            <h2>{{ isAmortization ? 'Tambah Entri Amortisasi' : 'Tambah Entri Penyusutan' }}</h2>
        </template>
            ...
            <div class="bg-white shadow-sm sm:rounded border border-gray-200 mb-6">
                <div class="p-6">
                    ...
                    <h3 class="text-lg font-semibold mb-4">
                        {{ isAmortization ? `Tambah Amortisasi untuk Aset ${asset?.name}` : `Tambah Penyusutan untuk Aset ${asset?.name}` }}
                    </h3>
