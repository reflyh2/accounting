...
function submit() {
    ...
    if (props.mode === 'edit' && props.transaction) {
        form.put(route('inventory.shipments.update', props.transaction.id), options);
    } else {
        form.post(route('inventory.shipments.store'), options);
    }
}
</script>
    ...
    <form @submit.prevent="submit" class="space-y-4">
        ...
        <div class="flex items-center space-x-3">
            <AppPrimaryButton type="submit" :disabled="form.processing">
                {{ props.mode === 'edit' ? 'Simpan Perubahan' : 'Simpan Pengeluaran' }}
            </AppPrimaryButton>
            <AppSecondaryButton :href="route('inventory.shipments.index')" as="a">
                Batal
            </AppSecondaryButton>
        </div>
    </form>
